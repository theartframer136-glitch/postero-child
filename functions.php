 ...
 * ================================================================ */
foreach (array('abandoned-cart', 'address-validation', 'fraud-detection', 'documents', 'marketplace', 'shipping', 'shipping-distance', 'kit-options', 'reels', 'cookie-consent', 'masonry', 'orientation-filter', 'blog-hub', 'analytics', 'chatbot', 'sales-count', 'review-enhancements', 'artist-profiles', 'banner-links', 'about-page', 'image-guard', 'fatal-recorder') as $af_mod) {
...
// ─────────────────────────────────────────────────────────────
// Header contact row: MY ACCOUNT and the email address overlapped at 100%
// browser zoom (owner recording, 2026-08-18) while looking correct at 90%.
//
// A row that is fine at one zoom and broken at another is a row whose items
// are laid out at fixed widths: at 90% everything happens to fit, at 100% the
// same boxes are wider than the space measured for them and the next item
// prints over the tail of the one before.
//
// The fix works from what is on the page rather than from theme class names:
// find the email link and the account link in the header, take the ancestor
// that holds both, and make THAT a proper flex line — nowrap, real gap, every
// child sized by its own content and none allowed to sit on its neighbour. If
// the line still cannot fit, the type is scaled down a little instead of
// letting anything overlap. Header only; nothing else on the page is touched.
// ─────────────────────────────────────────────────────────────
add_action('wp_footer', function () {
    if (is_admin()) return;
    ?>
<script id="af-hdr-row-fix">
(function(){
  function inFooter(el){
    return !!(el && el.closest && el.closest('footer, .site-footer, #footer, .elementor-location-footer, .footer'));
  }
  function findEmail(){
    var list = document.querySelectorAll('a[href^="mailto:"]');
    for (var i = 0; i < list.length; i++) if (!inFooter(list[i])) return list[i];
    return null;
  }
  function findAccount(){
    var list = document.querySelectorAll('a, span, div, li');
    for (var i = 0; i < list.length; i++) {
      var el = list[i];
      if (inFooter(el)) continue;
      var t = (el.textContent || '').trim();
      if (!/^my\s*account/i.test(t)) continue;
      if (t.length > 40) continue;                 // a wrapper, not the item
      return el;
    }
    return null;
  }
  function commonAncestor(a, b){
    var seen = [];
    for (var p = a; p; p = p.parentElement) seen.push(p);
    for (var q = b; q; q = q.parentElement) if (seen.indexOf(q) !== -1) return q;
    return null;
  }

  function fix(){
    var mail = findEmail(), acct = findAccount();
    if (!mail || !acct) return;
    var row = commonAncestor(mail, acct);
    if (!row || row === document.body) return;
    if (row.dataset.afRowFixed === '1' && row.offsetWidth === parseFloat(row.dataset.afRowW || 0)) return;
    row.dataset.afRowFixed = '1';
    row.dataset.afRowW = row.offsetWidth;
    ...
    sp(row, 'align-items', 'center');
    sp(row, 'gap', '16px');
    sp(row, 'column-gap', '16px');
    sp(row, 'overflow', 'visible');

    // every direct child stands on its own: no absolute placement, no negative
    // margin, no fixed width — the three ways one item lands on another
    for (var i = 0; i < row.children.length; i++) {
      var ch = row.children[i];
      var cs = window.getComputedStyle(ch);
      if (cs.position === 'absolute' || cs.position === 'fixed') sp(ch, 'position', 'relative');
      sp(ch, 'margin-left', '0');
      sp(ch, 'margin-right', '0');
      sp(ch, 'flex', '0 0 auto');
      sp(ch, 'width', 'auto');
      sp(ch, 'max-width', 'none');
      sp(ch, 'min-width', '0');
      sp(ch, 'white-space', 'nowrap');
      sp(ch, 'letter-spacing', 'normal');
      // the dropdown panel under MY ACCOUNT must stay absolute — it is not a
      // row item, it hangs below one
      var panels = ch.querySelectorAll('ul ul, .sub-menu, .elementor-nav-menu--dropdown');
      for (var d = 0; d < panels.length; d++) panels[d].style.removeProperty('position');
    }

    // If it still cannot fit, shrink the type rather than overlap. Steps down
    // to 88% only — past that the row stops being readable, and at that point
    // the honest answer is that the window is simply too narrow.
    var scale = 1;
    for (var pass = 0; pass < 4; pass++) {
      ...
      for (var j = 0; j < row.children.length; j++) {
        row.children[j].style.setProperty('font-size', (scale * 100) + '%', 'important');
      }
    }
  }
  ...
  [400, 1200, 2500].forEach(function(d){ setTimeout(fix, d); });
  // zoom changes fire resize: re-measure, since that is exactly the case that
  // was reported
