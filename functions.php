...
// ─────────────────────────────────────────────────────────────
// PHASE 3b — Homepage sections: New Arrivals, Reviews, Blog.
// Front page only. Rendered then JS-placed above the trust bar.
// Additive; reversible via this block.
// ─────────────────────────────────────────────────────────────
add_action('wp_footer', function() {
    if (!is_front_page() || !function_exists('wc_get_product')) return;
    ...
    if ($posts) {
        echo '<section class="af-home-sec af-blog"><div class="af-hs-inner">';
        echo '<div class="af-hs-head"><h2>From Our Blog</h2><a class="af-hs-more" href="/blog/">Read More →</a></div>';
        echo '<div class="af-blog-row">';
        foreach ($posts as $post) {
            $img = get_the_post_thumbnail_url($post->ID,'medium') ?: wc_placeholder_img_src('medium');
            echo '<a class="af-blog-card" href="'.esc_url(get_permalink($post->ID)).'">';
            echo '<div class="af-blog-img"><img src="'.esc_url($img).'" alt="'.esc_attr($post->post_title).'" loading="lazy"></div>';
            echo '<div class="af-blog-body"><span class="af-blog-date">'.esc_html(get_the_date('',$post->ID)).'</span>';
            echo '<h3>'.esc_html($post->post_title).'</h3>';
            echo '<p>'.esc_html(wp_trim_words($post->post_content,18)).'</p></div></a>';
        }
        echo '</div></div></section>';
    }
    ...
    $html = ob_get_clean();
    if (!$html) return;
    echo '<div id="af-home-extra">'.$html.'</div>';
    ?>
    <script>
    (function(){
    ...
    <style>
    .af-home-sec{padding:44px 16px;}
    .af-newarrivals{background:#fff;}
    .af-reviews{background:#faf7ef;}
    .af-blog{background:#fff;}
    .af-hs-inner{max-width:1240px;margin:0 auto;}
    .af-hs-head{display:flex;align-items:center;justify-content:space-between;margin:0 0 24px;}
    .af-hs-head h2{font-size:28px;font-weight:800;color:#1a1a1a;margin:0;}
    .af-hs-more{color:#c9a84c;font-weight:700;text-decoration:none;font-size:14px;white-space:nowrap;}
    .af-hs-more:hover{color:#a8872e;}
    /* New arrivals row */
    .af-hs-row{display:grid;grid-template-columns:repeat(4,1fr);gap:18px;}
    .af-hs-card{background:#fff;border:1px solid #eee;border-radius:12px;overflow:hidden;text-decoration:none;transition:box-shadow .25s,transform .25s;display:flex;flex-direction:column;}
    .af-hs-card:hover{box-shadow:0 8px 24px rgba(0,0,0,.12);transform:translateY(-3px);}
    .af-hs-imgwrap{aspect-ratio:1/1;overflow:hidden;background:#f4f4f4;}
    .af-hs-imgwrap img{width:100%;height:100%;object-fit:cover;transition:transform .4s;}
    .af-hs-card:hover .af-hs-imgwrap img{transform:scale(1.05);}
    .af-hs-info{padding:12px 14px;display:flex;flex-direction:column;gap:6px;}
    .af-hs-title{font-size:13.5px;font-weight:700;color:#1a1a1a;line-height:1.4;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;min-height:2.8em;}
    .af-hs-price{font-size:14px;font-weight:700;color:#c9a84c;}
    /* Reviews */
    .af-rv-row{display:grid;grid-template-columns:repeat(3,1fr);gap:18px;}
    .af-rv-card{background:#fff;border:1px solid #ece4cf;border-radius:12px;padding:20px;box-shadow:0 2px 10px rgba(0,0,0,.05);}
    .af-rv-stars{color:#c9a84c;font-size:16px;letter-spacing:2px;margin-bottom:10px;}
    .af-rv-text{font-size:14px;color:#444;line-height:1.6;margin:0 0 14px;font-style:italic;}
    .af-rv-meta strong{display:block;font-size:13px;color:#1a1a1a;}
    .af-rv-meta span{font-size:12px;color:#888;}
    /* Blog */
    .af-blog-row{display:grid;grid-template-columns:repeat(3,1fr);gap:20px;}
    .af-blog-card{background:#fff;border:1px solid #eee;border-radius:12px;overflow:hidden;text-decoration:none;transition:box-shadow .25s,transform .25s;}
    .af-blog-card:hover{box-shadow:0 8px 24px rgba(0,0,0,.12);transform:translateY(-3px);}
    .af-blog-img{aspect-ratio:16/9;overflow:hidden;background:#f4f4f4;}
    .af-blog-img img{width:100%;height:100%;object-fit:cover;}
    .af-blog-body{padding:16px;}
    .af-blog-date{font-size:12px;color:#c9a84c;font-weight:600;}
    .af-blog-body h3{font-size:16px;font-weight:700;color:#1a1a1a;margin:6px 0 8px;line-height:1.4;}
    .af-blog-body p{font-size:13px;color:#666;line-height:1.55;margin:0;}
    @media(max-width:900px){
      .af-hs-row{grid-template-columns:repeat(2,1fr);}
      .af-rv-row,.af-blog-row{grid-template-columns:1fr 1fr;}
    }
    @media(max-width:600px){
      .af-home-sec{padding:32px 14px;}
      .af-hs-head h2{font-size:22px;}
      .af-rv-row,.af-blog-row{grid-template-columns:1fr;}
    }
    </style>
    <?php
}, 26);
