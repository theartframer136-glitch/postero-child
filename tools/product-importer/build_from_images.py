...
def main():
    ...
    # Point the same pipeline at a DIFFERENT folder and a different store
    # section, without touching the built-in defaults. Everything else — the
    # vision naming, the store dedupe, the crash-safe cache — behaves exactly
    # as it does for the main run.
    ap.add_argument("--main-dir", action="append", default=None, metavar="PATH",
                    help="folder of artwork to read (repeatable); replaces the "
                         "built-in Final Edited Photos folders")
    ap.add_argument("--gallery-dir", default=None, metavar="PATH",
                    help='folder of wall/room photos to attach, or "none" for a '
                         "folder that has no gallery shots")
    ap.add_argument("--category", default=None, metavar="NAME",
                    help="file every product under THIS store category instead of "
                         '"Digital Canvas Prints" plus the vision-picked one')
    ap.add_argument("--out", default=None, metavar="FILE",
                    help="write this CSV instead of products_named.csv, so a "
                         "second collection never disturbs the first")
    args = ap.parse_args()

    global MAIN_DIRS, GALLERY_DIR, OUT, REPORT
    if args.main_dir:
        MAIN_DIRS = [Path(d) for d in args.main_dir]
    if args.gallery_dir is not None:
        GALLERY_DIR = None if args.gallery_dir.strip().lower() == "none" else Path(args.gallery_dir)
    if args.out:
        OUT = Path(args.out)
    ...
    if pending:
        ...
        if GALLERY_DIR is None or not Path(GALLERY_DIR).exists():
            # A collection can legitimately have no room shots. That is not an
            # error — it just means each product carries its own artwork alone.
            gallery, gfeat = [], {}
            print("  no gallery folder — main images only.\n")
        else:
    ...
    for i, (mp, picks) in enumerate(winners, 1):
        ...
        # --category names the section outright; the vision-picked category is
        # still recorded in the report, but it does not decide where the product
        # is filed.
        cats = args.category or ("Digital Canvas Prints" + (f"|{cat}" if cat else ""))
