...
/**
 * One-off: rotate the SECOND photo in the gallery of the "Moonlit Lotus
 * Radha" product (Art Code TA 04) 90 degrees counter-clockwise
 ...
 * Run: wp eval-file wp-content/themes/postero-child/tools/rotate-moonlit-lotus-radha-gallery2-ccw90.php --allow-root
 */
...
$rows = $wpdb->get_col( $wpdb->prepare(
    "SELECT post_id FROM {$wpdb->postmeta} WHERE meta_key='_taf_art_code' AND meta_value=%s", 'TA 04'
) );
if ( $rows ) $pid = (int) $rows[0];

if ( ! $pid ) {
    echo "Could not find a product with Art Code 'TA 04' — no changes made.\n";
    exit(0);
}
echo "Product #{$pid} \"" . get_the_title( $pid ) . "\"\n";

if ( get_post_meta( $pid, '_af_moonlit_lotus_radha_gallery2_rotated_ccw90', true ) ) {
...
try {
    ...
    update_post_meta( $pid, '_af_moonlit_lotus_radha_gallery2_rotated_ccw90', '1' );
